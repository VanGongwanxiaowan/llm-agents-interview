...
AI 应用程序需要[记忆](../../concepts/memory.md)来在多次交互之间共享上下文。在 LangGraph 中，您可以添加两种类型的记忆：

- [添加短期记忆](#add-short-term-memory)作为您智能体[状态](../../concepts/low_level.md#state)的一部分，以启用多轮对话。
- [添加长期记忆](#add-long-term-memory)来存储跨会话的用户特定或应用程序级数据。

## 添加短期记忆

**短期**记忆（线程级[持久性](../../concepts/persistence.md)）使智能体能够跟踪多轮对话。要添加短期记忆：
...
??? example "示例：使用 [MongoDB](https://pypi.org/project/langgraph-checkpoint-mongodb/) 检查点"
    ...
    !!! note "设置"
        ...
        要使用 MongoDB 检查点，您需要一个 MongoDB 集群。如果您还没有集群，请按照[此指南](https://www.mongodb.com/docs/guides/atlas/cluster/)创建一个集群。
...
如果您的图包含[子图](../../concepts/subgraphs.md)，您只需要在编译父图时提供检查点。LangGraph 会自动将检查点传播到子图。
...
如果您希望子图有自己的记忆，您可以使用适当的检查点选项编译它。这在[多智能体](../../concepts/multi_agent.md)系统中很有用，如果您希望智能体跟踪其内部消息历史。
...
LangGraph 允许智能体在工具内部访问其短期记忆（状态）。
...
要在执行期间修改智能体的短期记忆（状态），您可以直接从工具返回状态更新。这对于持久化中间结果或使信息对后续工具或提示可访问很有用。
...
```typescript title="智能体可以用来查找用户信息的工具"
import { tool } from "@langchain/core/tools";
import { z } from "zod";
...
在图的内存存储中启用语义搜索，让图智能体通过语义相似性在存储中搜索项目。
...
有关如何在 LangGraph 内存存储中使用语义搜索的更多信息，请参阅[此指南](../../cloud/deployment/semantic_search.md)。

## 管理短期记忆

启用[短期记忆](#add-short-term-memory)后，长对话可能会超过 LLM 的上下文窗口。常见的解决方案有：

- [修剪消息](#trim-messages)：删除前 N 个或后 N 个消息（在调用 LLM 之前）
- 从 LangGraph 状态中[永久删除消息](#delete-messages)
- [总结消息](#summarize-messages)：总结历史中的早期消息并用总结替换它们
- [管理检查点](#manage-checkpoints)来存储和检索消息历史
- 自定义策略（例如，消息过滤等）

这允许智能体跟踪对话而不超过 LLM 的上下文窗口。

### 修剪消息

大多数 LLM 都有最大支持的上下文窗口（以令牌为单位）。决定何时截断消息的一种方法是计算消息历史中的令牌数，并在接近该限制时截断。如果您使用 LangChain，您可以使用修剪消息实用程序并指定要从列表中保留的令牌数，以及用于处理边界的 `strategy`（例如，保留最后的 `maxTokens`）。

=== "在智能体中"

    :::python
    要在智能体中修剪消息历史，请使用 @[`pre_model_hook`][create_react_agent] 和 [`trim_messages`](https://python.langchain.com/api_reference/core/messages/langchain_core.messages.utils.trim_messages.html) 函数：
...
您可以从图状态中删除消息来管理消息历史。当您想要删除特定消息或清除整个消息历史时，这很有用。

:::python
要从图状态中删除消息，您可以使用 `RemoveMessage`。要使 `RemoveMessage` 工作，您需要使用带有 @[`add_messages`][add_messages] [reducer](../../concepts/low_level.md#reducers) 的状态键，如 [`MessagesState`](../../concepts/low_level.md#messagesstate)。
...
:::js
要从图状态中删除消息，您可以使用 `RemoveMessage`。要使 `RemoveMessage` 工作，您需要使用带有 @[`messagesStateReducer`][messagesStateReducer] [reducer](../../concepts/low_level.md#reducers) 的状态键，如 `MessagesZodState`。
